...
def test_access_other_api_orders_by_unauthorized_user(unauthorized_user):
    response_patch = unauthorized_user.patch(
        reverse("orders:order-detail", args={'pk': 1}), data={})
    response_put = unauthorized_user.patch(
        reverse("orders:order-detail", args={'pk': 1}), data={})
    response_list = unauthorized_user.get(reverse("orders:order-list"))
    response_retrieve = unauthorized_user.get(
        reverse("orders:order-detail", args={"pk": 1}))
...
def test_twice_create_order_for_one_car(user_client, user):
    car = CarFactory(status=CAR_STATUS_CHOICES.VERIFIED)
    ...
    response = user_client.post(
        reverse("orders:order-list"), data=data_to_send)
